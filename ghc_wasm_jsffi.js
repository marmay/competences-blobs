...
ZC0ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziComponentziWindowHostZC: () => (window.onbeforeunload = null),
ZC1ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziComponentziWindowHostZC: () => (window.onbeforeunload = function(e){return ''}),
ZC0ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziSvgEmbedziManagerZC: async ($1,$2) => (await MathJax.tex2svgPromise($1, {display: $2})),
ZC0ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: async ($1) => (new Uint8Array(await $1.arrayBuffer())),
ZC1ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => ($1 == null),
ZC2ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => ($1.size),
ZC3ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => ($1.type),
ZC4ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => ($1.name),
ZC5ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => ($1.files[0]),
ZC6ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1,$2,$3) => ((() => { const blob = new Blob([$1], { type: $2 }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = $3; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); })()),
ZC7ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1,$2) => (new Uint8Array(new Uint8Array(__exports.memory.buffer, $1, $2))),
ZC8ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1,$2) => ((new Uint8Array(__exports.memory.buffer, $2, $1.byteLength)).set($1)),
ZC9ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => ($1.byteLength),
ZC10ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziBinaryFFIZC: ($1) => (new Uint8Array($1)),
ZC0ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziIndexedDBZC: ($1) => ($1 == null),
ZC1ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziIndexedDBZC: async ($1,$2) => (await new Promise((resolve, reject) => { const tx = $1.transaction(['checkpoints'], 'readwrite'); const store = tx.objectStore('checkpoints'); store.delete($2); tx.oncomplete = () => resolve(); tx.onerror = (e) => reject(e.target.error); })),
ZC2ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziIndexedDBZC: async ($1,$2) => (await new Promise((resolve, reject) => { const tx = $1.transaction(['checkpoints'], 'readonly'); const store = tx.objectStore('checkpoints'); const req = store.get($2); req.onsuccess = () => resolve(req.result); req.onerror = (e) => reject(e.target.error); })),
ZC3ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziIndexedDBZC: async ($1,$2,$3) => (await new Promise((resolve, reject) => { const tx = $1.transaction(['checkpoints'], 'readwrite'); const store = tx.objectStore('checkpoints'); store.put($3, $2); tx.oncomplete = () => resolve(); tx.onerror = (e) => reject(e.target.error); })),
ZC4ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziIndexedDBZC: async () => (await new Promise((resolve, reject) => { const req = indexedDB.open('competences-checkpoints', 1); req.onupgradeneeded = (e) => { const db = e.target.result; if (!db.objectStoreNames.contains('checkpoints')) { db.createObjectStore('checkpoints'); } }; req.onsuccess = (e) => resolve(e.target.result); req.onerror = (e) => reject(e.target.error); })),
ZC5ZCcompetenceszmfrontendzm1zi4zi1zi0zminplaceZCCompetencesziFrontendziIndexedDBZC: async ($1) => (await crypto.subtle.digest('SHA-256', $1).then(buf => Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, '0')).join(''))),
